...
==== Generating the ecore model with the Execution Data and Execution Functions

This step makes use of link:http://melange-lang.org/[Melange] to automatically extend the AS
with ED and EF. Just select the melange file in the project explorer, right click on it and in the Melange menu, select generate all.
...
1. Select the MarkedGraphL language (/org.gemoc.sample.markedgraph.xdsml.markedgraphl/model/MarkedGraphL.ecore) and click __OK__.
...
3. On the last screen, complete the required fields:
    * The MarkedGraphL Ecore file has already been initialized, just click on __Browse__.
...
* Select the /org.gemoc.sample.markedgraph.xdsml.markedgraphl/model/MarkedGraphL.ecore (unfold the project structure)
* Click OK
