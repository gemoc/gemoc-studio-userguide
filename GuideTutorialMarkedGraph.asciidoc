...
Marked Graph syntax thanks to the wizard called "Create Animator Project for
language", then we will add a layer to describe the graphical animation of a
Marked Graph model.
...
First of all, we need to restart (__File > restart__) the second eclipse so that the new projects are available.
...
TODO: Describe how to corrected the generated org.gemoc.sample.markedgraph.animate project to make toggling breakpoints work.
