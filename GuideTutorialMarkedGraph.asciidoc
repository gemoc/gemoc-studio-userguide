...
Download the
link:images/MarkedGraph/files/org.gemoc.sample.markedgraph.zip[provided
...
replaced with the link:images/MarkedGraph/files/markedgraph.xtend[following text].
...
link:images/MarkedGraph/files/markedgraph.ecl[following code] (explanations on this code are
given bellow):
...
2. Then, it links them to EF form DSA --- when a DSE will occurs the
...
image::images/MarkedGraph/MG-animation-launch-wizard.png[Create GEMOC debug representation wizard]
...
link:images/MarkedGraph/files/org.gemoc.sample.markedgraph.sample.zip[org.gemoc.sample.markedgraph.sample]
...
To avoid the ECL file to become too large and to favor capitalization
and reuse of MoCC elements, it is possible (and encourage) to define
libraries.  Those libraries looks like the predefined ones that we
have already used like kernel.lib and CCSL.lib.
...
Sometimes, it may be easier to define the MoCC using an automata.  For
example, we can see a place as something on which we can read a token
(the output transition reads one token) or we can write a token (the
...
* The DSE part may be defined using a MoCCML library.  It favors reuse of
MoCC and clarity of ECL file.
* Rather than using CCSL syntax, it is possible to use constraint automata
