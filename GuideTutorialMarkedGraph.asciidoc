...
= Animate Marked Graph Models with the GEMOC Studio

TODO: Other title: Make Marked Graph models executable with the GEMOC
Studio
...
image::MarkedGraph/images/MG-XDSML-start.png[xDSML Editor]
...
image::MarkedGraph/images/MG-XDSML-EMF-browse.png[Browse to select AS]

Then, initialize the "Genmodel URI" and the "Root container model
element" thanks to the "Browse" and "Select" buttons.

image::MarkedGraph/images/MG-XDSML-EMF-done.png[xDSML Editor]
...
image::MarkedGraph/images/MG-XDSML-EMF-done.png[xDSML sirius]
...
token of each place (it is an ED).  When a transition is fired (EF),
one token is removed from each of its input places and one token is
added to each of irs output places.
