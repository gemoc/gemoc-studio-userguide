...
This tutorial will be illustrated with <<automata-domain,automata domain>>.
Possibilities of the GEMOC approach and the GEMOC Studio are progressively
demonstrated on this example through several increments:

1. First implementation of the deterministic Automata xDSML with a
focus on the DSA.

2. Second implementation with a focus on the MoCC (to show that a
balance must be found)

3. Provide a graphical visualization.

4. Consider non deterministic automata (evolution of the previous example)

5. Extend with Pushdown Automata (demonstrate the feedback mechanism

6. Integrate user defined code (connect the automata to a specific
graphical interface).
